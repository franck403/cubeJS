...
// SPEED FIX: tracked (JS-side) absolute position per port, updated the
// instant we SEND a move, not after we've confirmed it with the hub.
// This is what lets runMovement skip the pre-move readPositionOnce()
// round-trip entirely - we already know where the motor should be
// because we're the ones who told it to go there. The trade-off: if a
// move silently stalls/truncates mid-sequence, trackedPos and the
// hub's real position can drift apart until alignAfterSequence() (end
// of spikeCube()) catches and corrects it in one shot instead of on
// every single move.
let trackedPos = { left: {}, right: {} };

function resetTrackedPos() {
    trackedPos = { left: {}, right: {} };
}
...
async function runMovement(move, sleep = 220, noCube = false) {
    ...
    // SPEED FIX: no more pre-move readPositionOnce() round-trip. We trust
    // our own JS-side tracked position (updated the instant we send a
    // move, below) instead of asking the hub and waiting for a reply
    // before we're even allowed to send the command. This is what was
    // costing ~1 extra UART round-trip per move (the 5s -> 16s
    // regression). First move of a fresh connection defaults to 0
    // (matches the hub's own power-on absolute_position reference).
    if (trackedPos[which][port] === undefined) trackedPos[which][port] = 0;
    const currentPos = trackedPos[which][port];
    const targetPos = currentPos - deg0;
    trackedPos[which][port] = targetPos; // optimistic update, corrected later if wrong
    ...
    // SPEED FIX: confirmation is now a single non-blocking-ish check, not
    // a retry-with-resend loop per move. We still poll for "did it
    // arrive" (catches genuine stalls worth knowing about mid-run) but
    // we no longer re-send + re-wait on timeout here - that full
    // recovery now happens once, for everything at once, in
    // alignAfterSequence() after the whole sequence finishes. This is
    // the other half of the speed fix: a slow/missed confirm no longer
    // blocks the NEXT move.
    waitForPosition(which, port, targetPos).then(result => {
        if (!result.ok) {
            log(`WARN: ${move} on port ${port} (${which}) did not confirm within ${profile.posTimeoutMs}ms - will fix in end-of-sequence alignment`);
        }
    });

    // Still respect a minimum visual/audible pacing between moves so the
    // light matrix + sound don't overlap awkwardly.
    await sleepT(Math.min(wait, profile.pacingFloorMs));
}

/**
 * Runs once at the END of a sequence (spikeCube), not per-move. Reads the
 * REAL absolute position of every port that was touched and, if it
 * doesn't match what we optimistically tracked in JS, sends ONE
 * corrective run_to_absolute_position to re-sync. This is the trade we
 * made for speed: instead of paying a UART round-trip before/after every
 * single move to guarantee alignment, we accept that trackedPos can
 * drift from hub-truth mid-sequence (silent stall, missed confirm) and
 * pay for exactly one correction pass at the end.
 */
async function alignAfterSequence() {
    const profile = getSpeedProfile();
    const corrections = [];
    ...
    for (const which of ['left', 'right']) {
        const writer = which === 'left' ? leftWriter : rightWriter;
        if (!writer) continue;
        const ports = Object.keys(trackedPos[which]);
        for (const port of ports) {
            const expected = trackedPos[which][port];
            const actual = await readPositionOnce(which, port);
            if (actual === null) {
                log(`ALIGN: ${which} port ${port} - could not read position, skipping correction`);
                continue;
            }
            const drift = actual - expected;
            if (Math.abs(drift) > profile.posToleranceDeg) {
                corrections.push({ which, port, writer, expected, actual, drift });
            }
        }
    }

    if (corrections.length === 0) {
        log('ALIGN: all ports on target, no correction needed');
        return;
    }

    log(`ALIGN: correcting ${corrections.length} port(s) - ${corrections.map(c => `${c.which}${c.port}(${c.drift > 0 ? '+' : ''}${c.drift}°)`).join(', ')}`);

    await Promise.all(corrections.map(async ({ which, port, writer, expected }) => {
        const cmd = `motor.run_to_absolute_position(port.${port}, ${expected}, ${profile.moveVelocity}, acceleration=${profile.moveAccel}, deceleration=${profile.moveDecel});\n`;
        await sendLine(writer, cmd);
        await waitForPosition(which, port, expected, 1500);
        trackedPos[which][port] = expected;
    }));
}
...
async function spikeCube(moves, sleeped = 150) {
    ...
    // SPEED FIX: this is the one-shot alignment pass replacing the old
    // per-move pre-read. Runs after the timer stops so it doesn't count
    // against solve/scramble time, but before batteries so any drift is
    // fixed before the next sequence starts from a known-good position.
    await alignAfterSequence();
    await sleepT(200)
