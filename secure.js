async function se() {
  try {
    if (localStorage.getItem('gncl_accepted') === 'true' || ('caches' in window && await caches.open('gncl-cache').then(c => c.match('/LICENSE').then(Boolean).catch(() => false)))) return true;
  } catch (e) { }
  let text = null;
  try {
    const resp = await fetch('/LICENSE.md', { cache: 'no-store' });
...
window.se = se;
if (document.readyState === 'loading') { document.addEventListener('DOMContentLoaded', () => { window.se() }, { once: true }) } else window.se();